  ...
  <app-card :title="title" body-padding="0">
    ...
    <div v-if="!currentRider">
      ...
      <span v-if="this.authUser.type ==='admin' || this.authUser.type ==='superadmin'">
        ...
        <button
          :class="
            active === 'partial-verified'
              ? 'btn btn-round btn-success btn-xs title-right-action'
              : 'btn btn-round btn-xs title-right-action'
          "
          @click="getParitallyVerifiedRider"
          title="Refresh"
        >
          <i class="material-icons">shield</i>
          Partially Verified
          <sup>
            <span class="badge" v-if="counts.partiallyverified > 0">{{
    ...
    <app-table-sortable
      :columns="columns"
      :rows="rows"
      :searchUrl="'/admin/driver/get-drivers?name='"
      v-if="!currentRider"
    >
      <template slot-scope="{ row }" style="background-color: red">
        ...
        <td>
          ...
          <span
            ...
            v-if="row.partialStatus == true"
            ><i class="material-icons">shield</i></span
          >
          <span v-else><span
            style="cursor: none"
            title="Verified"
        ...
        <td>
          ...
          <small
            ...
            ><span v-if="row.subscription">{{ row.subscription }}</span><span v-else>"No Subscription"</span></small
          >
        </td>
        <td>
        ...
        <td>
          <div class="col-md-3" v-if="authUser.type === 'admin' || authUser.type === 'superadmin' ">
            <app-actions
          ...
          <div class="col-md-3" v-if="!row.blocked && (authUser.type === 'admin' || authUser.type === 'unit-head') ">
            <button
              type="button"
              title="Block Rider"
              class="btn btn-danger btn-ajax"
          ...
          <div class="col-md-3" v-if="!row.verified && !row.partialStatus">
            <button
              type="button"
              title="Partially Verify Now"
              class="btn btn-warning btn-ajax"
              @click="partiallyVerifyDriver(row)"
            >
              <i class="material-icons">shield</i>
            </button>
          </div>
...
<script>
import Driver from "@utils/models/Driver";
...
export default {
  ...
  data() {
    return {
      ...
      title: "All <b>Riders</b> need to Verify",
  ...
  methods: {
    ...
    changeSubscription() {
      axios
        .post("/admin/driver/change-subscription-type", {
    ...
    async partiallyVerifyDriver(row) {
      if (confirm("Are you sure? You cant undo this action.")) {
        let response = await this.model.partiallyVerify(row.id);
        if (response === "success") {
          // this.reset();
          row.partialStatus = true;
          alertMessage("Rider Partially Verified.");
        }
      }
    },
    ...
    updateAssociation(riderId) {
      ...
      axios
        .post("/admin/driver/change-associated-status", {
          id: riderId,
    ...
    getIncompleteRider() {
      axios.get("/admin/driver/incomplete").then((response) => {
        this.rows = { data: [], links: {}, meta: {} };
    ...
    getParitallyVerifiedRider() {
      axios.get("/admin/driver/partial-verified").then((response) => {
        this.rows = { data: [], links: {}, meta: {} };
        this.rows.data = response.data.data;
        this.rows.meta = response.data.meta;
        this.rows.links = response.data.links;
        this.blockedOnly = false;
        this.blacklistedOnly = false;
        this.active = "partial-verified";
      });
    },
    async reset() {
      this.currentRider = "";
      this.title = "All <b>Riders</b> need to Verify";
    ...
    updateLicense(row = null) {
      if (row) {
        ...
        axios
          .post("/admin/driver/update-expiry-date", {
    ...
    async counter() {
      ...
      axios.get("/admin/driver/partial-verified").then((response) => {
        this.counts.partiallyverified = response.data.meta.total;
      });
    },
