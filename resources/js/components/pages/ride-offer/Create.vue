<template>
  <app-card :title="'Ride Offer Discount Configuration'">
      ...
      <div class="row">
        <div class="col-md-6">
          <input-text
            ...
            type="number"
            name="no_of_rides"
            v-model="form.noOfRides"
            v-validate="'required'"
            :error-text="errors.first('no_of_rides')"
            required
          ></input-text>
        </div>
        <div class="col-md-6">
...
import RideOfferConf from "@utils/models/RideOfferConf";
import { store, save } from "@utils/mixins/Crud";

export default {
  name: "RideOfferConfCreate",
  mixins: [store, save],
  data() {
    return {
      edit: false,
    ...
    async getData() {
      axios.get(this.model.indexUrl).then((response) => {
        if(response.data.data){
          let {
            offerTitle,noOfRides,discount,enabled,from,to,
            from_date,
            from_time,
            to_date,
            to_time,     
          ...
          this.form = new Form({
            offerTitle,noOfRides,discount,from,to,enabled
          });
