...
class KhaltiResponse extends PaymentResponse implements ResponseContract
{
    ...
    {
        return env('KHALTI_MODE', 'test') == 'live'
            ? 'live_secret_key_6b30b9c24f1e4816b4bcec65fcc0d377'
            : 'test_secret_key_8d5d601bd84f48aa80e74bd427f134d6';
    }
    ...
    protected function getPaymentDetail(string $token): array
    {
        ...
        Log::channel('payment')->info("--------------------------------------------------------------------------------------------");
        Log::channel('payment')->info("--------------------------------------------------------------------------------------------");
        Log::channel('payment')->info("                  Khalti Response:-" . json_encode($result));
        Log::channel('payment')->info("                  User Phone:" . auth()->guard('api')->user()->phone . " Name:" . auth()->guard('api')->user()->name);
        Log::channel('payment')->info("                  Using Secret:" . $this->getSecretKey());
        Log::channel('payment')->info("--------------------------------------------------------------------------------------------");
        Log::channel('payment')->info("--------------------------------------------------------------------------------------------");
