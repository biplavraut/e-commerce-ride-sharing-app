...
class PaymentLogController extends CommonController
{
    ...
    {
        $user = User::where('first_name', 'LIKE', $request->name . '%')->orWhere('last_name', 'LIKE', $request->name . '%')->orWhere('phone', 'LIKE', $request->name . '%')->first();
        if ($user) {
            $logs = $this->paymentLogService->query()->where('user_id', $user->id)
                ->paginate($this->paginationLimit);
        } else {
            $logs = $this->paymentLogService->query()->where('action', 'LIKE', $request->name . '%')
