...
class ProductController extends CommonController
{
    ...
    {
        try {
            $vendor = $this->vendorService->query()->whereHas('products')->with('products')->findOrFail($request->vendorId);
            return ProductResource::collection($vendor->products()->where('hide', 0)->where('verified', 1)->paginate($this->paginationLimit))->additional(['status' => true, 'message' => "", 'statusCode' => 200], 200);
        } catch (\Throwable $th) {
    ...
    public function getProduct(Request $request)
    {
        ...
        try {
            $product = $this->productService->query()
                ...
                ->where('hide', 0)
                ->Where('verified', 1)
                ->with(['vendor', 'category', 'qas', 'reviews'])
