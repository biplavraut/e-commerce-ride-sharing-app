...
class TripController extends Controller
{
    ...
    public function tripRequest(Request $request)
    {
        ...
        $myAccidentedTrip = $user->trips()->where('status', 'accident')->where('done', 0)->count();

        if ($myAccidentedTrip > 0) {
            return failureResponse("Accident was detected previously. Please contact to gogo20 support for further assistance.", 422, 422);
        }
    ...
    public function tripCancelled(Request $request)
    {
        ...
        if ($trip->status !== "completed" && $trip->status !== "cancelled") {
            ...
            $this->doTheWorkFirebaseWork($trip);
    ...
    public function tripStatus(Request $request)
    {
        ...
        if ($request->status == 'dispute') {
            ...
            $trip->driver->status()->update(['status' => 'ongoing']);
    ...
    {
        $notification = new PushNotification(
            $driver->devices->pluck('device_token')->toArray(),
            [
    ...
    public function tripPayment(Request $request)
    {
        ...
        if ($request->donationTrust && $request->donation) {
            $this->donationProcess($request->donationTrust, $request->donation, $user);
            $trip['donationAmount'] = $request->donation;
        }
        $trip->update(['payment_mode' => $request->paymentMode ?? 'cod']);
        // $this->firebaseRTD($user, $trip);
        // $this->paymentSettlement($trip, $trip->driver, $request->donation ?? 0);

        return successResponse("Operation Success.", 201, 201);
        //newly added end
    }
    ...
    public function paymentSettlement($trip, $rider, $donation)
    {

        if ($rider->settlement) {
            $rider->settlement->update(['donation_amount' => ($rider->settlement->donation_amount + $donation)]);
        } else {
            $rider->settlement()->create(['donation_amount' => $donation]);
        }
    }
    ...
    public function firebaseTripDelRTD($tripId)
    {
        ...
        try {
            $riderTempTrips = json_decode($this->firebase->get(env('RIDER_TEMP_PATH', 'riderTempTrips/') . $tripId));
            ...
            $log = TripRequestLog::where('trip_id', $tripId)->first();
            if ($log) {
                try {
                    foreach ($log->rider_list as  $rider) {
                        $data = json_decode($this->firebase->get(env('RIDER_TRIP_PATH', 'riderTrips/') . $rider));
                        if ($data->trip->id == $tripId) {
                            $this->firebase->delete(env('RIDER_TRIP_PATH', 'riderTrips/') . $rider);
                        }
                    }
                } catch (\Throwable $th) {
                    //throw $th;
    ...
    public function recall(Request $request)
    {
        ...
        if (!$trip = Trip::where('user_id', $user->id)->Where('id', $request->tripId)->Where('driver_id', null)->first()) {
            return failureResponse("Trip not found.", 404, 404);
        }

        $trip->update(['created_at' => now()]);

        $tripAlgorithm = new TripRequestAlgorithm($user, $trip, $request->preference, true);
        $response = $tripAlgorithm->beginTransaction();
    ...
    public function doTheWorkFirebaseWork($trip)
    {
        try {
            $riderTemps = json_decode($this->firebase->get(env('RIDER_TEMP_PATH', 'riderTempTrips/') . $trip->id));
            foreach ($riderTemps as $key => $value) {
                $this->firebase->delete(env('RIDER_TEMP_PATH', 'riderTempTrips/') . $trip->id . '/' . $key);
